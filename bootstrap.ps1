...
"@
    ...
    try {
        ...
        foreach ($profile in $config.chromeProfiles) {
            $profileColor = if (!($profile.color)) {
                get-random -minimum 0 -maximum 255 + "," get-random -minimum 0 -maximum 255 + "," get-random -minimum 0 -maximum 255
            } else {
                $profile.color
            }
            New-Item -Path "$env:TEMP\Chrome\$($profile.name)" -ItemType Directory -Force | out-null
            $linkPath        =  "$env:USERPROFILE\Desktop\$($profile.name).lnk"
            $targetPath      = "$env:ProgramFiles\Google\Chrome\Application\chrome.exe"
            $link            = (New-Object -ComObject WScript.Shell).CreateShortcut($linkPath)
            $link.TargetPath = $targetPath
            $link.Arguments = "--user-data-dir=$env:TEMP\Chrome\$($profile.name) --install-autogenerated-theme=$profileColor --homepage=""https://msportals.io"" --no-initial-navigation"
            [void]$link.Save()
        }
    }
    catch {
