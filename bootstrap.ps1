...
"@
    ...
    try {
        ...
        foreach ($chromeProfile in $config.chromeProfiles) {
            $profileColor = if (!($chromeProfile.color)) {
                get-random -minimum 0 -maximum 255 + "," get-random -minimum 0 -maximum 255 + "," get-random -minimum 0 -maximum 255
            } else {
                $chromeProfile.color
            }
            New-Item -Path "$env:TEMP\Chrome\$($chromeProfile.name)" -ItemType Directory -Force | out-null
            $linkPath        =  "$env:USERPROFILE\Desktop\$($chromeProfile.name).lnk"
            $targetPath      = "$env:ProgramFiles\Google\Chrome\Application\chrome.exe"
            $link            = (New-Object -ComObject WScript.Shell).CreateShortcut($linkPath)
            $link.TargetPath = $targetPath
            $link.Arguments = "--user-data-dir=$env:TEMP\Chrome\$($chromeProfile.name) --install-autogenerated-theme=$profileColor --homepage=""https://msportals.io"" --no-initial-navigation"
            [void]$link.Save()
        }
    }
    catch {
...
try {
    ...
    Set-Content -Value $profileText -Path $profile -ErrorAction stop
...
[Environment]::SetEnvironmentVariable("SandboxSource", $config.SandboxSource, 'Machine')
[Environment]::SetEnvironmentVariable("SandboxShared", $config.SandboxShared, 'Machine')
New-Item -Path HKCU:\Software\Sandbox
New-ItemProperty -Path HKCU:\SOFTWARE\Sandbox -Name SandboxSource -PropertyType String -Value $config.SandboxSource -Force
New-ItemProperty -Path HKCU:\SOFTWARE\Sandbox -Name SandboxShared -PropertyType String -Value  $config.SandboxShared -Force

# BGInfo
try {
    Invoke-WebRequest -Uri 'https://download.sysinternals.com/files/BGInfo.zip' -OutFile "$($env:TEMP)\BGInfo.zip"
    unblock-file "$($env:TEMP)\BGInfo.zip"
    expand-archive "$($env:TEMP)\BGInfo.zip" "$($env:TEMP)"
    start-process "$($env:TEMP)\bginfo.exe" -ArgumentList "$($config.SandboxSource)\sandbox.bgi /timer:0 /nolicprompt /silent" -Wait
}
catch {
    Write-Error "Error Installing BGInfo" -errorAction Continue
}
